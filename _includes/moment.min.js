...
0,0,"unix");addFormatToken("x",0,0,"valueOf");addRegexToken("x",matchSigned);addRegexToken("X",matchTimestamp);addParseToken("X",function(input,array,config){config._d=new Date(parseFloat(input,10)*1E3)});addParseToken("x",function(input,array,config){config._d=new Date(toInt(input))});hooks.version="2.17.1";setHookCallback(createLocal);hooks.fn=proto;hooks.min=min;hooks.max=max;hooks.now=now;hooks.utc=createUTC;hooks.unix=createUnix;hooks.months=listMonths;hooks.isDate=isDate;hooks.locale=getSetGlobalLocale;
...
getSetRelativeTimeThreshold;hooks.calendarFormat=getCalendarFormat;hooks.prototype=proto;return hooks});(function(){var day,formats,hour,initialize,minute,second,week;second=1E3;minute=6E4;hour=36E5;day=864E5;week=6048E5;formats={seconds:{"short":"s","long":" sec"},minutes:{"short":"m","long":" min"},hours:{"short":"h","long":" hr"},days:{"short":"d","long":" day"}};initialize=function(moment){var twitterFormat;twitterFormat=function(format){var diff,num,unit,unitStr;diff=Math.abs(this.diff(moment()));unit=null;num=null;if(diff<=second){unit="seconds";num=1}else if(diff<minute)unit="seconds";else if(diff<
hour)unit="minutes";else if(diff<day)unit="hours";else if(format==="short")if(diff<week)unit="days";else return this.format("M/D/YY");else return this.format("MMM D");if(!(num&&unit))num=moment.duration(diff)[unit]();unitStr=unit=formats[unit][format];if(format==="long"&&num>1)unitStr+="s";return num+unitStr};moment.fn.twitterLong=function(){return twitterFormat.call(this,"long")};moment.fn.twitter=moment.fn.twitterShort=function(){return twitterFormat.call(this,"short")};return moment};if(typeof define===
"function"&&define.amd)define("moment-twitter",["moment"],function(moment){return this.moment=initialize(moment)});else if(typeof module!=="undefined")module.exports=initialize(require("moment"));else if(typeof window!=="undefined"&&window.moment)this.moment=initialize(this.moment)}).call(this);